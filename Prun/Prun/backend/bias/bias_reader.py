...
LastEditTime: 2021-10-18 16:47:38
...
def bias_read(job ,id, tag, index,npzPath):
    
    if tag == "contrast":
        data = searchDataPath("bias",job,id,npzPath)
        return data
    ...
    try:
        ...
        step = r["step"].tolist()
        pos = splitP(v)
        return {"bias":v,"shape":shape,"step":step,"index":int(index)+1,"tag":"success","op":tag,"split":pos}
    except Exception:
        return {"bias":"none","index":int(index)+1,"tag":"success","op":tag}
